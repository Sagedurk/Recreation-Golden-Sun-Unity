...
public class InteractableEditor : Editor 
{
    ...
    void OnDialogue(Interactable interactable)
    {
        ...
        for (int i = 0; i < interactable.listOfDialogueBoxes.Count; i++)
        {
            ...
            SerializedProperty dialogueAction = dialogueInstance.FindPropertyRelative("dialogueAction");
            SerializedProperty actionName = dialogueInstance.FindPropertyRelative("actionName");
            ...
            if (listFoldouts[i])
            {
                ...
                EditorGUILayout.PropertyField(dialogueAction);

                if (dialogueAction.enumValueIndex != (int)DialogueMaster.DialogueAction.NONE)
                {
